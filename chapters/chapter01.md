...
実際のところ、**リーダーシップとマネジメントは、一つのスキルセットとして捉えるのではなく、連携し合う2つの要素として捉えるべき**だと私は考えます。EMの役割は、この2つのバランスをとりながら、「技術」と「人」と「ビジネス」の交差点に立つことです。
...
重要なのは、これらの3つの領域は相互に関連しており、バランスを取ることが必要だということです。しかし、実際のEMの仕事では、状況に応じて優先順位が変わります：

**スタートアップ期（〜20名）：**
- Technology: 40%（技術的な基盤作り）
- Process: 20%（最小限のプロセス）
- People: 40%（少数精鋭のチーム作り）

**成長期（20〜100名）：**
- Technology: 30%（スケーラビリティ確保）
- Process: 35%（プロセスの標準化）
- People: 35%（採用と育成）

**成熟期（100名〜）：**
- Technology: 25%（アーキテクチャガバナンス）
- Process: 35%（効率化と最適化）
- People: 40%（組織開発と文化醸成）
...
心理的安全性というフレーズは非常に使い勝手がよく、上司とメンバー、もしくはメンバー同士の対立やコミュニケーション不全を「心理的安全性がない」と言い換えてしまうことが多いように思います。私も概ねそう捉えていました。
...
実際のEMの一日はどのようなものでしょうか。ある典型的な火曜日を例に見てみます：
